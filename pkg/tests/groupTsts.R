...
##'  \code{\link{generate.reference.dataset}}
##' @param iteration see \code{\link{generate.reference.dataset}}
##' @param do2S ditto
##' @return a \code{\link{list}} with components \code{x} and \code{beta} similar to those
##' of \code{generate.reference.dataset()} and \code{p.vec} and \code{s0.vec} which are the
...
rXbBlock <- function(p.vec, s0.vec, n, ..., iteration = NA, do2S = FALSE) {
    ...
    grL <-
        lapply(1:np, function(k)
            generate.reference.dataset(n = n, p = p.vec[k], s0 = s0.vec[k],
...
##' @param rXb a list, e.g. resulting from rXb (== generate.reference.dataset) or rXbBlock()
...
d1 <- generate.reference.dataset(n = 64, p =  16, s0 =  7, iteration=NA, do2S = FALSE)
d2 <- generate.reference.dataset(n = 64, p = 256, s0 = 12, iteration=NA, do2S = FALSE)
...
if(!exists("generate.reference.dataset", mode="function")) ## "old hdi" -- tests should give the same (?!) -- TODO
...
showSys.time(
    gb2g <- groupBound(x = d2xy$x, y = d2xy$y, group = pvec2grp(d2B$p.vec), silent=TRUE)
)
stopifnot(
    as.vector(gb1) == 0
   ,
