##This file contains functions that calculate the Z vectors as nodewise lasso residuals
##see http://arxiv.org/abs/1110.2563
##and can also calculate the Thetahat matrix as in http://arxiv.org/abs/1303.0518
                                ...
                                lambdatuningfactor=1)
{
  ...
  ##10-fold cv is done over all nodewise regressions to calculate the error for the different lambda
  cvlambdas <- cv.nodewise.bestlambda(lambdas=lambdas,x=x,parallel=parallel,ncores=ncores,oldschool=oldschool)
  ...
  if(wantTheta)
    {
      ...
      Z <- score.getZforlambda(x=x,lambda=bestlambda,parallel=parallel,ncores=ncores,oldschool=oldschool)
      out <- Z
    }
...
score.getThetaforlambda <- function(x,lambda,parallel=FALSE,ncores=8,oldschool=FALSE,verbose=FALSE,
                                    oldtausq=TRUE)
{
  ## Purpose:
  ## This function is for calculating Thetahat once the desired tuning parameter is known
  ## ----------------------------------------------------------------------
  ## Arguments:
  ## ----------------------------------------------------------------------
  ## Author: Ruben Dezeure, Date: 27 Nov 2012 (initial version),
      ...
      for(i in 1:p)
        {
          ...
          if(oldtausq)
            {##possibly quite incorrect,it ignores the intercept, but intercept is small anyways. Initial simulations show little difference.
              T2[i] <- as.numeric(crossprod(x[,i])/n - x[,i] %*% (x[,-i] %*% coeffs)/n)
            }else{   
              ##print("now doing the new way of calculating tau^2")
              T2[i] <- as.numeric((x[,i] %*% (x[,i] - predict(glmnetfit,x[,-i],s=lambda)))/n)
            }
        }
...
score.getZforlambda <- function(x,lambda,parallel=FALSE,ncores=8,oldschool=FALSE)
  {
    ## Purpose:
    ## This function is for calculating Z once the desired tuning parameter is known
    ## ----------------------------------------------------------------------
    ## Arguments:
    ## ----------------------------------------------------------------------
    ## Author: Ruben Dezeure, Date: 27 Nov 2012 (initial version),
    ...
    if(oldschool)
      {
        ...
        if(parallel)
          {
            Z <- mcmapply(score.getZforlambda.unitfunction,i=1:p,x=list(x=x),lambda=lambda,mc.cores=ncores)
            
          }else{
            Z <- mapply(score.getZforlambda.unitfunction,i=1:p,x=list(x=x),lambda=lambda)
          }
      }
  ...
  ## Author: Ruben Dezeure, Date: 27 Nov 2012 (initial version),
    ...
    for(i in 1:K)
        {#loop over the test sets
            ...
            glmnetfit <- glmnet(x[!whichj,-c,drop=FALSE],x[!whichj,c,drop=FALSE],lambda=lambdas)
...
cv.nodewise.bestlambda <- function(lambdas,x,K=10,parallel=FALSE,ncores=8,oldschool=FALSE)
{
  ...
  ## A second value of the tuning parameter, always bigger or equal to the former,
  ## is returned which is calculated by allowing the cv error to increase by the amount of
  ...
  if(oldschool)
    {
      ...
      totalerr <- numeric(l)
      for(c in 1:p)
        {#loop over the nodewise regressions
          for(i in 1:K)
            {#loop over the test sets
              whichj <- dataselects == i#the test part of the data
