...
calculate.pvalue.for.cluster <- function(hh,p,pvalfunction,clusterextractfunction,
                                         verbose=FALSE)
{##Remark: not the cleanest code yet
  ...
  start <- TRUE
  for(nclust in 1:p)
    {##test the subclusters of the significant current clusters
      if(verbose)
      ...
      if(!all.signif)
        {
          cut.level[!upper.signif] <- 0##ignore clusters that are not significant above
        }
      clusters.this.level <- sapply(setdiff(unique(cut.level),0),FUN="==",cut.level)
      ...
      if(!(identical(cluster.lengths,old.cluster.lengths)))
        {##we went down the tree one level for those clusters we are interested in
          ...
          if(verbose)
            print(paste("total number of clusters at this level = " ,ncol(clusters.this.level),sep=""))
          ...
          for(j in 1:length(current.clusts))
            {##check if current cluster is a child
              for(i in 1:length(upper.clust))
                {##check if it has a child
                  if(all(is.element(current.clusts[[j]],upper.clust[[i]])) && (start || !(length(current.clusts[[j]]) == length(upper.clust[[i]]))))
          ...
          if(verbose)
            print(paste("there are ",sum(are.children)," children that we will test.",sep=""))
          
          if(sum(are.children)>0)##else: we have not encountered children in this level of tree
            {
              ##if there are children it is only possible that there are two children clusters of
              ##one upper cluster that has children, this is due to the iteration over cutree
              ##TODO
              if(!start)
                {#add leftchild and rightchild
                  ##we assume always that nchildren =2 at this point!
                  if(mean(((1:length(ord)))[ord %in% current.clusts[[which(are.children)[2]]]] / p) < mean(((1:length(ord)))[ord %in% current.clusts[[which(are.children)[1]]]] / p))
                    {
                      left <- iter + 1
                      right <- iter
                    }else{
              ...
              for(i in which(pvals <= 0.05))
                {
                  ...
                  upper.clust.ind[[tmp.iter]] <- iter - sum(are.children) + (i-1)#index in clusters[[]]
                  tmp.iter <- tmp.iter + 1
                }
              upper.signif <- 1:p %in% unique(unlist(upper.clust))
              ##done
              if(length(upper.clust) == 0)##there are no more upper significant clusters
                {
                  if(verbose)
                    {
  ...
  clusterGroupTest <- function(hcloutput,
    ...
    {##optional argument: hcloutput = the result from a hclust call
      ...
      clusterextractfunction <- function(nclust)
        {##function to extract the correct level of the tree where the number of clusters = nclust
          cutree(hh,k=nclust)
        }
      pvalfunction <- function(clusters.to.test,
                               nclust)
      ...
      out <- calculate.pvalue.for.cluster(hh=hh,
                                          ...
                                          clusterextractfunction=clusterextractfunction)
