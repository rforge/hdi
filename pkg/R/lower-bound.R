...
getLowerBoundNode <- function(x, y, me, sel, resmat, groupsl, alpha = 0.05,
  ...
  return(list(resmat = resmat, groupsl = groupsl))
}
...
lowerGroupBoundWithPrediction <- function(x, y, group, mfact, pred,
  ...
  for (m in 1:M){
    ...
    solB <- solveLP(cvec, bvec, Amat, lpSolve = lpSolve)
    lvec[m] <- sum(penalty * solB$solution)
  }

  Amat <- cbind(x, -Z)
  Amat <- rbind(Amat,-Amat, c(0 * penalty, rep(1, length(lvec))),
                c(penalty, -lvec))
...
lowerGroupBound <- function(x, y, group, alpha = 0.05, nsplit = 11, s = 10,
  ...
  for (splitc in 1:nsplit){
    if(setseed) set.seed( useseed <- splitc +201)
    ...
    if(!is.null(s)){
      nout <- length(outsam)
      A <- cbind(pred/sqrt(sum(pred^2)), matrix(rnorm(nout*(s-1)),nrow=nout))
      for (sc in 2:s){
        tmpP <- A[,1:(sc-1),drop=FALSE]
        tmp <- A[,sc] - tmpP %*% solve(t(tmpP)%*%tmpP +
                                       0.00001*diag(sc-1), t(tmpP) %*%A [,sc])
      ...
      TGsplit <- lowerGroupBoundWithPrediction(A %*% x[outsam,],
                                               ...
                                               else NULL,lpSolve=lpSolve)
   }else{
     mfact <- getmfact(nrow(x),1-alpha/2)
     TGsplit <- lowerGroupBoundWithPrediction(x[outsam,],y[outsam],
                                              group,mfact,pred,intercept=TRUE,
                                              useseed=if(setseed)useseed else
                                              NULL,lpSolve=lpSolve)
   }
   if(!silent) cat("\n   // lower l1-norm bound", signif(TGsplit,2))
   if(setseed) set.seed(oldseed)
...
clusterLowerGroupBound <- function(x, y, method = "average", dist = NULL,
